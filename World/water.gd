...
@onready var water_collision_shape: CollisionShape2D = $Area2D/CollisionShape2D 
...
@onready var movable_player: Node2D = get_tree().root.find_child("Player", true, false) 

var hoover_audio_player: AudioStreamPlayer2D
var is_hoovering: bool = false
...
func _ready() -> void:
	hoover_audio_player = AudioStreamPlayer2D.new()
	add_child(hoover_audio_player)
	hoover_audio_player.stream = load("res://assets/SFX/Hoover.wav")
	...
	if event.is_action_pressed("interact"):
		...
		if has_hoover and not hoover_is_filled:
			...
			if water_suck_animation:
				...
				is_hoovering = true
				_play_hoover_sound_loop()
				
				if interact_label:
					interact_label.visible = false 
		elif not has_hoover: 
			inspect_water()
		elif has_hoover and hoover_is_filled: 
			if interact_label:
...
func inspect_water() -> void:
	...
	if world and world.has_method("say_ai"):
		await world.say_ai("If only I could paint.", 4.0)
...
func _on_water_suck_animation_finished() -> void:
		...
		is_hoovering = false
		water_suck_animation.stop() 
		water_suck_animation.frame = water_suck_animation.sprite_frames.get_frame_count("Water") - 1 
		
		var world = find_parent("World")
		var ai_cam = world.find_child("FeedingCam", true, false)
		...
		if world and world.has_method("say_ai"):
			await world.say_ai("Give me that!", 3.0)
		...
		Global.world_state["hoover_filled_with_water"] = true 
		Global.world_state["water_sucked"] = true 
...
func _play_hoover_sound_loop() -> void:
	while is_hoovering:
		hoover_audio_player.pitch_scale = randf_range(0.9, 1.1)
		hoover_audio_player.play()
		await hoover_audio_player.finished
		if not is_hoovering: break
